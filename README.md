...
### How to properly use this program.
Each file is designated to the naming conventions of Simulation_part[Number], Go through each part and adjust variables shown to retrieve outputs.
...
The probability only goes through one iteration of a simulation.

## Part 2:
Since Part 1 only went through one iteration, i want to dedicate this part for several iterations and finding the probability of that - it would result in the reason that having a large sample size is always good.

### Part 2 results:

Should suspect results stay at a certain probability of turning into a werewolf due to a set of travelers and werewolves summoned into the simulator.
